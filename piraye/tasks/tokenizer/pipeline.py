...
class TokenizerPipeline(Tokenizer):
    ...
    @staticmethod
    def __merge(text: str, previous_tokens: List[Token], new_tokens: List[Token]) -> List[Token]:
        """
        ...
        while i < len(previous_tokens) or j < len(new_tokens):
            ...
            if p_end <= n_start:
                if current_token:
                    current_token = TokenizerPipeline.__extend_token(current_token, prev, text, prev)
